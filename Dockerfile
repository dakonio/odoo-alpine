...
RUN unzip -qq ${ODOO_VERSION}.zip && cd odoo-${ODOO_VERSION} && \
    ...
    sed -i "/gevent==21.8.0 ; sys_platform != 'win32' and python_version > '3.9' and python_version <= '3.10'  # (Jammy)/d" requirements.txt && \
    sed -i "/greenlet==1.1.2 ; sys_platform != 'win32' and python_version  > '3.9' and python_version <= '3.10'  # (Jammy)/d" requirements.txt && \
    sed -i "/lxml==4.6.5 ; python_version <= '3.10'  # min version = 4.5.0 (Focal - with security backports)/d" requirements.txt && \
    sed -i "/reportlab==3.5.59 ; python_version <= '3.10'  # version < 3.5.54 are not compatible with Pillow 8.1.2 and 3.5.59 is bullseye/d" requirements.txt && \
